...
public class Controles implements Screen {
    ...
    public Controles(MyGdxGame game, int nivel) {
        lvl = nivel;
        this.game = game;
        guiCam = new OrthographicCamera();
        guiCam.setToOrtho(false, 800, 480);
        testTable = new Table();
        testTable.setBackground(new TextureRegionDrawable(new TextureRegion(new Texture("controlCorrer.png"))));
    ...
    public void update() {
        ...
        if (Gdx.input.justTouched()) {
            game.setScreen(new Control2(this.game, lvl));
        }
