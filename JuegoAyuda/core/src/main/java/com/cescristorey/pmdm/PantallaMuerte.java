...
public class PantallaMuerte implements Screen {
    ...
    public PantallaMuerte(MyGdxGame game) {
        this.game = game;
        guiCam = new OrthographicCamera();
        guiCam.setToOrtho(false, 800, 480);
        testTable = new Table();
        testTable.setBackground(new TextureRegionDrawable(new TextureRegion(new Texture("PantallaMuerte.png"))));
    ...
    public void update() {
        ...
        if (Gdx.input.justTouched()) {
            game.setScreen(new PantallaInicio(this.game));
        }
